...
CppMlpErrorCode cppmlp_read_mnist_meta(const char* filename, CppMlpReadDims* dims) {
	...
	uint32_t imageRows; // uint32_t type important for reading!
	uint32_t imageCols;
	...
	// 08 bits, 03 dimensions
		...
		dataIfstream.read(reinterpret_cast<char*>(&imageRows), sizeof(uint32_t));
		imageRows = swapEndian(imageRows);

		dataIfstream.read(reinterpret_cast<char*>(&imageCols), sizeof(uint32_t));
	...
	dims->imageCols = imageCols;
	dims->imageRows = imageRows;
	dims->numImages = numImages;
	return CPPMLP_GOOD;
}


CppMlpErrorCode cppmlp_read_mnist(const char* filename, void* outputPtr, const int cppmlp_readtype) {

	CppMlpReadDims dims;
	CppMlpErrorCode err = cppmlp_read_mnist_meta(filename, &dims);
	if (err != CPPMLP_GOOD) { return err; }
...
CppMlpErrorCode cppmlp_init(CppMlpHndl* hndl, const char* directory) {
	CppMlpErrorCode err;
	CppMlpReadDims xDims;
	err = cppmlp_read_mnist_meta((std::string(directory) + "/train-images.idx3-ubyte").c_str(), &xDims);
	if (err != CPPMLP_GOOD) {
		assert(false);
		return err;
	}

	CPUMatrix<float> xCpu = CPUMatrix<float>(xDims.numImages, xDims.imageCols * xDims.imageRows);
	...
	GPUMatrix<float> xGpu{ xCpu };

	CppMlpReadDims yDims;
	err = cppmlp_read_mnist_meta((std::string(directory) + "/train-labels.idx1-ubyte").c_str(), &yDims);
	if (err != CPPMLP_GOOD) {
		assert(false);
		return err;
	}
	CPUMatrix<int> yCpu = CPUMatrix<int>(yDims.numImages, yDims.imageCols * yDims.imageRows);
	err = cppmlp_read_mnist(
	...
	void* mlpAsToken;
	try {
		mlpAsToken = new MLP{ xGpu, yGpu, hiddenSize, BATCH_SIZE, LR, EPOCHS };
	}
...
int main() {
	...
	CppMlpHndl hndl;
	// No need to check CppMlpErrorCode here because we'll just break on assert
	cppmlp_init(&hndl, "assets.ignored");
